...
def index():
    ...
    # dashboard of user's stocks and shares
    stocks = db.execute('SELECT symbol, SUM(shares) as total_shares FROM transactions WHERE user_id = :user_id GROUP BY symbol HAVING total_shares > 0',
                        user_id=session['user_id'])

    # User's funds balance
    ...
    # Loop stocks and add price & total value
    for stock in stocks:
...
def history():
    ...
    # Check user's transactions in database
...
def register():
    ...
    if request.method == 'POST':
        ...
        # Remember logged in user and redirect
        session['user_id'] = rows[0]['id']
